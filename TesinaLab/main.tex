...
La seguente relazione si basa sull'analisi dell'architettura di comunicazione via radio tra radiocomando e microcontrollore. 
...
I timer permettono di misurare accuratamente gli intervalli temporali, requisito fondamentale per acquisire e decodificare i segnali della ricevente. Per la gestione dell'\textit{Input Capture} (sfruttando i canali PA0 e PA3 del TIM5) il modulo è alimentato da un clock di base pari a 64 MHz.

Per ottenere una risoluzione temporale ideale per la misurazione, si è deciso di ridurre la frequenza di conteggio operativa del timer a 1 MHz tramite l'utilizzo del registro \textit{Prescaler} (PSC). 

Dal punto di vista architetturale, l'hardware del microcontrollore STM32 divide la frequenza di partenza per un fattore pari al valore contenuto nel registro PSC, incrementato di un'unità. Pertanto, per ottenere una divisione effettiva per 64, il parametro da impostare a livello software deve essere 63. La logica di calcolo è descritta dalla seguente equazione:

$$PSC = \frac{f_{clock}}{f_{target}} - 1 = \frac{64.000.000}{1.000.000} - 1 = 63$$
...
Il segnale PWM in uscita dal ricevitore del radiocomando presenta una frequenza fissa (tipicamente 50 Hz, pari a un periodo di 20 ms). Pertanto, il calcolo della frequenza, poiché costante,risulta irrilevante ai fini del controllo dinamico del veicolo. L'informazione vitale legata al comando impartito dal pilota risiede unicamente nella larghezza dell'impulso alto ($T_{on}$), che varia proporzionalmente al movimento delle levette in un intervallo tipicamente compreso tra 1ms e 2ms.
...
\noindent La logica di acquisizione è gestita all'interno della funzione di callback \verb|HAL_TIM_IC_CaptureCal|\allowbreak\verb|lback|. Poiché la medesima funzione di interruzione viene richiamata dall'hardware per ogni variazione di stato logico del pin, è necessario l'ausilio di un flag di stato (come \verb|Is_First_Cap|\allowbreak\verb|tured_Steering|) per fungere da memoria e permettere al sistema di distinguere se il fronte rilevato corrisponda all'inizio o alla fine dell'impulso.

\noindent Nello specifico, al verificarsi del fronte di salita elettrico dell'onda PWM, il sistema salva il valore attuale del contatore hardware, imposta il flag per indicare che la misurazione è iniziata, e commuta contestualmente la sensibilità del canale sul fronte opposto. Al successivo fronte di discesa elettrico, il contatore viene letto nuovamente. È fondamentale precisare che questa commutazione di lettura (salita e discesa) si riferisce esclusivamente alla forma d'onda elettrica del segnale a 50 Hz inviato dalla ricevente, la quale si ripete ciclicamente e indipendentemente dalla direzione in cui il pilota sta muovendo i comandi.

\noindent Poiché il timer opera con un clock effettivo di 1 MHz, la differenza algebrica tra le due letture restituisce la durata dell'impulso alto ($T_{on}$) espressa direttamente in microsecondi. Tale valore viene salvato nelle variabili globali (\verb|Xa| e \verb|Xb|) per essere impiegato come setpoint, a patto che superi il controllo di sicurezza implementato per scartare letture anomale esterne al range nominale (600--2400 µs).
...
L'obiettivo del sistema di acquisizione è misurare esclusivamente il tempo in cui il segnale PWM in ingresso si trova a livello logico alto ($T_{on}$). Questa precisa durata temporale, infatti, è direttamente proporzionale alla posizione fisica della levetta del radiocomando mossa dal pilota e costituisce il valore numerico che verrà assegnato come setpoint.

\noindent A differenza delle classiche implementazioni hardware che prevedono l'azzeramento forzato del contatore del timer ad ogni fronte di salita (metodologia che impedisce la lettura simultanea di più canali e costringe all'utilizzo di molteplici timer), in questo progetto si è optato per un approccio software "non distruttivo". Come si evince dalla configurazione di sistema in STM32CubeMX, il timer dedicato alla decodifica (TIM5) è impiegato sui canali 1 e 4 in modalità \textit{Input Capture direct mode}, ma il suo contatore viene fatto avanzare in modo continuo (fino al suo limite massimo a 32 bit, pari a 4294967295) senza mai subire reset forzati. 

\noindent All'attivazione dell'interrupt, la callback si limita a "fotografare" il valore assoluto del registro tramite la funzione \verb|HAL_TIM_ReadCapturedValue|. La durata dell'impulso alto viene così ricavata puramente per differenza matematica, sottraendo il timestamp memorizzato al momento della salita da quello acquisito al momento della discesa. 

\noindent Il risultato di questa sottrazione rappresenta l'esatta durata del segnale alto generato dalla levetta e viene salvato nelle variabili di sistema \verb|Xa| e \verb|Xb|. Per prevenire errori di calcolo nel raro caso in cui avvenga un overflow del timer proprio durante la lettura di un impulso, l'algoritmo include un controllo condizionale che compensa il riavvio del conteggio sommando il limite massimo del registro.

\noindent Questa architettura software garantisce numerosi vantaggi:
\begin{itemize}
...
L'approccio implementato per l'avvio e la configurazione del sistema all'interno del file \verb|main.c| si basa su una logica asincrona rispetto all'arrivo dei segnali del radiocomando e non bloccante per il normale flusso di esecuzione del programma. Anziché arrestare la CPU in un ciclo di attesa finché non viene ricevuto e decodificato il primo impulso PWM valido, le operazioni iniziali si limitano ad avviare le periferiche necessarie in rapida sequenza. 
L'intera logica di sincronizzazione e allineamento al fronte d'onda viene infatti delegata a una macchina a stati finiti (gestita tramite flag software) implementata direttamente all'interno della routine di interruzione hardware. In particolare, per quanto riguarda l'acquisizione dei segnali provenienti dal radiocomando (\verb|Xa| per la trazione e \verb|Xb| per lo sterzo), è sufficiente abilitare all'avvio gli interrupt legati all'Input Capture sui canali 1 e 4 del TIM5. 
Contestualmente, viene avviato il timer di base (TIM6) che scandisce il ciclo di controllo primario ogni 10 ms. In questo modo, il ciclo principale (il \verb|while(1)|) e le routine di controllo sono liberi di avviarsi istantaneamente, elaborando i dati in background man mano che gli interrupt hardware notificano, in modo asincrono, il completamento di una nuova lettura.
...
\noindent Nelle prossime sezioni si riportano i file, le definizioni e le strutture utilizzate per il controllo della trazione e dello sterzo, commentandone le costanti e i prototipi di funzione.
...
\noindent Il codice appena illustrato si occupa di inizializzare la comunicazione seriale configurando la porta dedicata e il corretto baud rate. In particolare, la comunicazione tra il microcontrollore STM32 e MATLAB avviene mediante interfaccia UART, sfruttando la funzione \texttt{printf()} reindirizzata verso la periferica seriale tramite la routine \texttt{\_write()}. Di seguito sono riportati i frammenti di codice sorgente che implementano la temporizzazione e il conseguente invio dei dati:
...
//USART3 -> ST_Link UART for DEBUG with USB
...
\noindent In questo modo ogni chiamata a \texttt{printf()} viene trasmessa sulla porta seriale e resa disponibile al software MATLAB. Durante il ciclo di controllo, con periodo di campionamento pari a 10 ms, il firmware acquisisce la velocità reale dall'encoder e calcola la corrispondente velocità di riferimento.

\noindent Sul fronte della ricezione, lo \textit{script} intercetta questo flusso continuo di dati. Dapprima predispone un grafico dinamico e, tramite un ciclo iterativo, legge i pacchetti in arrivo isolando i valori della velocità di riferimento e di quella reale. Oltre a tracciare le due curve in tempo reale, il codice calcola e aggiorna dinamicamente a schermo il valore dell'Errore Quadratico Medio (MSE), permettendo di valutare istantaneamente la bontà dell'inseguimento del \textit{setpoint}, per poi arrestarsi automaticamente al raggiungimento del limite prefissato di campioni.
\vspace{0.4cm}
...
\noindent Nel primo grafico, relativo all'utilizzo del doppio filtro in cascata, si può notare come il segnale misurato (curva blu) risulti fluido. Tuttavia, questo eccessivo filtraggio introduce un grave ritardo di fase nell'anello di controllo. A causa di questo ritardo, il controllore reagisce in ritardo all'errore, innescando un'ampia e lenta oscillazione sottosmorzata. Nel secondo grafico, che illustra la risposta con il singolo filtro FIR, il comportamento dinamico risulta nettamente superiore. Il tempo di risposta del veicolo migliora e, inoltre, una volta giunto a regime, il sistema oscilla con frequenza maggiore. L'utilizzo di un singolo filtro FIR in retroazione si è dunque confermata la scelta decisiva.
...
\subsection{Variazione velocità angolare}
