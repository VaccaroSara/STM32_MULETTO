...
\subsection{Trazione}
Il controllo della velocità longitudinale è stato affidato a un regolatore di tipo PI (Proporzionale-Integrale), i cui parametri sono stati definiti empiricamente e affinati sul campo per garantire una risposta rapida e priva di errore a regime.
Le costanti di guadagno ottimali, il tempo di campionamento dell'anello di controllo e i limiti di saturazione simmetrica sono stati definiti all'interno del firmware come segue:

\begin{lstlisting}[language=Cpp, caption=""]
//TRACTION PID
...
\begin{lstlisting}[language=Matlab, caption={Codice in MATLAB}, basicstyle=\ttfamily\small, breaklines=true]
